...
var DurationFmt = function DurationFmt(options) {
  var sync = true;
                ...
                onLoad: ilib.bind(this, function (fmtHM) {
                  ...
                  new DateFmt({
                    ...
                    onLoad: ilib.bind(this, function (fmtHMS) {
                      this.timeFmtHMS = fmtHMS; // munge with the template to make sure that the hours are not formatted mod 12
...
DurationFmt.prototype.format = function (components) {
  var i,
      ...
      secondlast = true,
      str = "";
  list = DurationFmt.complist[this.style]; //for (i = 0; i < list.length; i++) {
  ...
  for (i = list.length - 1; i >= 0; i--) {
    ...
    if (typeof components[list[i]] !== 'undefined' && components[list[i]] != 0) {
      ...
      str = this.components[list[i]].formatChoice(components[list[i]], {
        num: this._mapDigits(components[list[i]])
      }) + str;
    }
  }
