...
var Charmap = function Charmap(options) {
  var sync = true,
...
Charmap.prototype = {
   ...
   * @private
   */
  writeNative: function writeNative(array, start, value) {
   ...
   * @private
   */
  writeNativeString: function writeNativeString(array, start, string) {
   ...
   * @private
   */
  _calcExpansionFactor: function _calcExpansionFactor() {
    var factor = 1;
   ...
   * @private
   */
  dealWithMissingChar: function dealWithMissingChar(c) {
    var seq = "";
   ...
   */
  mapToNative: function mapToNative(string) {
    if (!string) {
    ...
    var str = string instanceof IString ? string : new IString(string); // use IString's iterator so that we take care of walking through
   ...
   */
  mapToUnicode: function mapToUnicode(bytes) {
    var ret = "";
    var c,
        i = 0;

    while (i < bytes.length) {
      c = bytes[i]; // the default algorithm is plain old ASCII
