...
var UTF8 = function UTF8(options) {
  options = options || {
    sync: true
  };

  if (_typeof(options.charset) === "object" && options.charset instanceof Charset) {
...
UTF8.prototype.mapToNative = function (str) {
  ...
  var istr = str instanceof IString ? str : new IString(str); // step through the surrogate pairs as single code points by using
  // IString's iterator 

  var it = istr.iterator(); // multiply by 4 because the max size of a UTF-8 char is 4 bytes, so
