...
var Measurement = require("./Measurement.js"); // for converting ilib lengths to the ones that are supported in cldr
...
var UnitFmt = function UnitFmt(options) {
  var sync = true,
  ...
  this.roundingMode = options.roundingMode || "halfup"; // ensure that the plural rules are loaded before we proceed
    ...
    Utils.loadData({
      ...
      callback: ilib.bind(this, function (format) {
        ...
        if (this.usage && format.usages && format.usages[this.usage]) {
          ...
          this.usageInfo = format.usages[this.usage]; // default settings for this usage, but don't override the options that were passed in
...
UnitFmt.prototype = {
  /** @private */
  _initNumFmt: function _initNumFmt(sync, loadParams, options, callback) {
    new NumFmt({
      locale: this.locale,
  ...
  _initFormatters: function _initFormatters(sync, loadParams, options, callback) {
  ...
  /** @private */
  _init: function _init(sync, loadParams, callback) {
    if (this.style === "list" || this.usageInfo && this.usageInfo.systems && (this.usageInfo.systems.metric.style === "list" || this.usageInfo.systems.uscustomary.style === "list" || this.usageInfo.systems.imperial.style === "list")) {
      new ListFmt({
        locale: this.locale,
        style: "unit",
        sync: sync,
   ...
   * @private
   */
  _format: function _format(u, system) {
    var unit = u.getUnit() === "long-ton" ? "ton" : u.getUnit();
    var formatted = new IString(this.template[unit]); // make sure to use the right plural rules
    ...
    formatted = formatted.formatChoice(rounded, {
      n: this.numFmt[system].format(u.amount)
    });
   ...
   */
  format: function format(measurement) {
    var u = measurement,
        system,
        listStyle;
