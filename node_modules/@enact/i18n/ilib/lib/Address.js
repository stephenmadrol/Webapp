...
var Address = function Address(freeformAddress, options) {
  var address;
  ...
  this.locale = this.locale || new Locale(); // initialize from an already parsed object

  if (_typeof(freeformAddress) === 'object') {
    /**
...
Address.prototype = {
   ...
   * @private
   */
  _loadCountries: function _loadCountries(onLoad) {
   ...
   * @private
   */
  _loadCtrynames: function _loadCtrynames(onLoad) {
    Utils.loadData({
      name: "ctrynames.json",
      object: "Address",
      locale: this.locale,
   ...
   */
  _findDest: function _findDest(ctrynames) {
    var match;
   ...
   */
  _determineDest: function _determineDest(localizedCountries, callback) {
    var match;
    /*
   ...
   * @private
   */
  _parseAddress: function _parseAddress() {
    // clean it up first
    var i,
        asianChars = 0,
        latinChars = 0,
        ...
        matchFunction,
        match,
        fieldNumber; // for locales that support both latin and asian character addresses, 
    ...
    if (this.info && this.info.multiformat) {
      for (var j = 0; j < this.lines.length; j++) {
        ...
        while (it.hasNext()) {
          ...
          if (isIdeo(c) || CType.withinRange(c, "hangul") || CType.withinRange(c, 'katakana') || CType.withinRange(c, 'hiragana') || CType.withinRange(c, 'bopomofo')) {
            asianChars++;
      ...
      infoFields = this.info.fields[this.format]; // //console.log("multiformat locale: format is now " + this.format);
    } else {
      startAt = this.info && this.info.startAt || "end";
      infoFields = this.info.fields;
    }

    this.compare = startAt === "end" ? this.endsWith : this.startsWith; //console.log("this.lines is: " + JSON.stringify(this.lines));

    for (i = 0; i < infoFields.length && this.lines.length > 0; i++) {
      field = infoFields[i];
      this.removeEmptyLines(this.lines); //console.log("Searching for field " + field.name);
      ...
      if (field.pattern) {
        ...
        this[field.name] = this.lines.splice(fieldNumber, 1)[0].trim(); //console.log("typeof(this[field.name]) is " + typeof(this[field.name]) + " and value is " + JSON.stringify(this[field.name]));
      }
    ...
    this.lines = undefined; //console.log("final result is " + JSON.stringify(this));
  },
   ...
   */
  _findCountry: function _findCountry(name) {
    var start = -1,
        match,
        line = 0;
  ...
  endsWith: function endsWith(subject, query) {
    var start = subject.length - query.length,
        i,
        pat; //console.log("endsWith: checking " + query + " against " + subject);
  ...
  startsWith: function startsWith(subject, query) {
    var i; // //console.log("startsWith: checking " + query + " against " + subject);
  ...
  removeEmptyLines: function removeEmptyLines(arr) {
    var i = 0;
  ...
  matchRegExp: function matchRegExp(address, line, expression, matchGroup, startAt) {
    var lastMatch,
        match,
        ret = {},
        last; //console.log("searching for regexp " + expression.source + " in line " + line);
    ...
    if (match && match !== null) {
      ...
      if (match[matchGroup] !== undefined) {
        ...
        last = startAt === 'end' ? line.lastIndexOf(match[matchGroup]) : line.indexOf(match[matchGroup]); //console.log("last is " + last);
        ...
        ret.line = ret.line.trim(); //console.log("found match " + ret.match + " from matchgroup " + matchGroup + " and rest of line is " + ret.line);
  ...
  matchPattern: function matchPattern(address, line, pattern, matchGroup) {
    var start,
        j,
        ret = {}; //console.log("searching in line " + line);
