...
var NameFmt = function NameFmt(options) {
  var sync = true;
  this.style = "short";
  this.loadParams = {};
...
NameFmt.prototype = {
   ...
   * @protected
   */
  _adjoinAuxillaries: function _adjoinAuxillaries(parts, namePrefix) {
    var start, i, prefixArray, prefix, prefixLower; //console.info("_adjoinAuxillaries: finding and adjoining aux words in " + parts.join(' '));
      ...
      for (start = 0; start < parts.length - 1; start++) {
        for (i = parts.length; i > start; i--) {
          ...
          if (prefixLower in this.info.auxillaries) {
            ...
            parts.splice(start, i + 1 - start, prefixArray.concat(parts[i])); //console.info("_adjoinAuxillaries: Found! New parts list is " + JSON.stringify(parts));
  ...
  format: function format(name) {
    ...
    if (typeof name.isAsianName === 'boolean' && !name.isAsianName || Name._isEuroName([name.givenName, name.middleName, name.familyName].join(""), currentLanguage)) {
      isAsianName = false; // this is a euro name, even if the locale is asian

      modified = name.clone(); // handle the case where there is no space if there is punctuation in the suffix like ", Phd". 
