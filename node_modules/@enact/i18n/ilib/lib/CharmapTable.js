...
var CharmapTable = function CharmapTable(options) {
  var sync = true,
      loadParams = undefined; // console.log("CharmapTable: constructor with options: " + JSON.stringify(options));

  this.parent.call(this, options);
  this.charsetName = "ISO-8859-15";

  if (options) {
    if (_typeof(options.charset) === "object") {
...
CharmapTable.prototype._trieWalk = function (trie, array, start) {
  function isValue(node) {
    return typeof node === 'string' || typeof node === 'number' || _typeof(node) === 'object' && ilib.isArray(node);
  }
