...
var Collator = function Collator(options) {
  var sync = true,
      loadParams = undefined,
      useNative = true; // defaults
...
Collator.prototype = {
   ...
   */
  _pack: function _pack(arr, offset) {
    var value = 0;
   ...
   */
  _packRule: function _packRule(rule, offset) {
   ...
   * @private
   */
  _addChars: function _addChars(str, offset) {
   ...
   * @private
   */
  _addRules: function _addRules(rules, start) {
    var p;
  ...
  /**
      * @private
      */
  _initCollation: function _initCollation(rules) {
    var rule = this.style;
    ...
    if (typeof this.collation.inherit !== 'undefined') {
      for (var i = 0; i < this.collation.inherit.length; i++) {
        ...
        rule = _typeof(col) === 'object' ? col.name : col;
   ...
   * @private
   */
  _basicCompare: function _basicCompare(left, right) {
    ...
    if (this.numeric) {
      var lvalue = new INumber(left, {
        locale: this.locale
      });
      var rvalue = new INumber(right, {
        locale: this.locale
      });
