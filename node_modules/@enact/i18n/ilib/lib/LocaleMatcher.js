...
var componentWeights = [0.5, // language
0.2, // script
0.25, // region
0.05 // variant
];
...
var LocaleMatcher = function LocaleMatcher(options) {
  var sync = true,
...
LocaleMatcher.prototype = {
   ...
   * Do the work
   */
  _getLikelyLocale: function _getLikelyLocale(locale) {
   ...
   */
  getLikelyLocale: function getLikelyLocale() {
  ...
  match: function match(locale) {
    ...
    if (this.locale.region === other.region) {
      ...
      if (!this.locale.region || !other.region) {
        ...
        var containers = this.info.territoryContainmentReverse[this.locale.region] || []; // end at 1 because 0 is "001" which is "the whole world" -- which is not useful
   ...
   */
  getMacroLanguage: function getMacroLanguage() {
   ...
   */
  _getRegionContainment: function _getRegionContainment(region) {
   ...
   */
  getRegionContainment: function getRegionContainment() {
   ...
   */
  smallestCommonRegion: function smallestCommonRegion(otherLocale) {
    ...
    var otherRegions = this._getRegionContainment(otherRegion); // region containment arrays are arranged from largest to smallest, so start
