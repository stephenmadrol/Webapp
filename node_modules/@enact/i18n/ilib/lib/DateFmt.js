...
var DateFmt = function DateFmt(options) {
  var arr,
      i,
      bad,
      sync = true,
  ...
  if (options.length) {
    if (options.length === 'short' || options.length === 'medium' || options.length === 'long' || options.length === 'full') {
  ...
  if (typeof options.meridiems !== 'undefined' && (options.meridiems === "chinese" || options.meridiems === "gregorian" || options.meridiems === "ethiopic")) {
  ...
  new LocaleInfo(this.locale, {
    ...
    onLoad: ilib.bind(this, function (li) {
      this.locinfo = li; // get the default calendar name from the locale, and if the locale doesn't define
...
DateFmt.prototype = {
   ...
   * 	}} formats
   */
  _initTemplate: function _initTemplate(formats) {
    if (formats[this.calName]) {
      var name = formats[this.calName]; // may be an alias to another calendar type
      ...
      this.componentOrder = this._getFormat(this.formats.date, "dmy", "l").replace(/[^dMy]/g, "").replace(/y+/, "y").replace(/d+/, "d").replace(/M+/, "m");
    } else {
   ...
   * @protected
   */
  _massageTemplate: function _massageTemplate() {
    var i;
    ...
    var digits; // set up the mapping to native or alternate digits if necessary
   ...
   */
  _tokenize: function _tokenize(template) {
    var i = 0,
        start,
        ch,
        letter,
        arr = []; // console.log("_tokenize: tokenizing template " + template);
    ...
    if (template) {
      while (i < template.length) {
        ...
        if (ch === "'") {
          ...
          i++; // escaped string - push as-is, then dequote later
          ...
          letter = template.charAt(i); // console.log("found letters " + letter);
        ...
        arr.push(template.substring(start, i)); // console.log("start is " + start + " i is " + i + " and substr is " + template.substring(start,i));
      }
    }
   ...
   */
  getDateComponentOrder: function getDateComponentOrder() {
   ...
   */
  getDateComponents: function getDateComponents() {
   ...
   */
  getTimeComponents: function getTimeComponents() {
   ...
   */
  getMeridiemsRange: function getMeridiemsRange() {
    var result;

    var _getSysString = function _getSysString(key) {
   ...
   * @private
   */
  _getTemplate: function _getTemplate(prefix, calendar) {
   ...
   */
  getMonthsOfYear: function getMonthsOfYear(options) {
   ...
   */
  getDaysOfWeek: function getDaysOfWeek(options) {
    var length = options && options.length || this.getLength(),
   ...
   */
  _formatTemplate: function _formatTemplate(date, templateArr) {
    var i,
        key,
        temp,
        tz,
    ...
    for (i = 0; i < templateArr.length; i++) {
      switch (templateArr[i]) {
        ...
        */
        ...
        case 'ccc':
        case 'cccc':
          key = templateArr[i] + date.getDayOfWeek(); //console.log("finding " + key + " in the resources");
        ...
        case 'O':
          ...
          str += temp.formatChoice(date.day, {
            num: date.day
          });
          break;
   ...
   */
  format: function format(dateLike) {
    ...
    var dateZoneName = date.timezone || "local"; // convert to the time zone of this formatter before formatting
   ...
   */
  formatRelative: function formatRelative(reference, date) {
    ...
    if (_typeof(reference) !== 'object' || !reference.getCalendar || reference.getCalendar() !== this.calName || _typeof(date) !== 'object' || !date.getCalendar || date.getCalendar() !== this.calName) {
