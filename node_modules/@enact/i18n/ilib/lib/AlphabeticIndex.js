...
var AlphabeticIndex = function AlphabeticIndex(options) {
  this.sync = true;
  this.loadParams = {};
...
AlphabeticIndex.prototype._updateCollationMap = function () {
  this.mixedCollationMap = new Array(); // we just loaded it, so it should already be in the cache,
...
AlphabeticIndex.prototype._getKeyByValue = function (value, validMapNum) {
  ...
  for (i = 0; i < this.indexUnits.length; i++) {
    if (this.collationMap[this.indexUnits[i]][0] === value[0]) {
...
AlphabeticIndex.prototype.getAllBuckets = function () {
  ...
  for (i = 0; i < this.index.length; i++) {
    if (this.inherit && this.mixedCollationMap[0].indexUnits.indexOf(this.index[i].label) === -1) {
...
AlphabeticIndex.prototype.getBucket = function (element) {
  ...
  if (collationValue) {
    ...
    } else if (_typeof(collationValue[0]) === 'object') {
