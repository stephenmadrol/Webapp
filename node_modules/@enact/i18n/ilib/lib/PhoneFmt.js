...
var PhoneFmt = function PhoneFmt(options) {
  this.sync = true;
  this.styleName = 'default', this.loadParams = {};
...
PhoneFmt.prototype = {
  ...
  _substituteDigits: function _substituteDigits(part, formats, mustUseAll) {
    var formatString,
        ...
        partIndex = 0,
        templates,
        i; // console.info("Globalization.Phone._substituteDigits: typeof(formats) is " + typeof(formats));

    if (!part) {
      return formatted;
    }

    if (_typeof(formats) === "object") {
      ...
      formatString = templates[part.length - 1]; // console.info("Globalization.Phone._substituteDigits: formats is an Array: " + JSON.stringify(formats));
    } else {
   ...
   */
  _getStyle: function _getStyle(name, fmtdata) {
   ...
   */
  _doFormat: function _doFormat(number, options, startField, locale, fmtdata, callback) {
    var sync = true,
        loadParams = {},
        temp,
        templates,
    ...
    styleTemplates = this._getStyle(style, fmtdata); // console.log("Style ends up being " + style + " and using subtype " + (isWhole ? "whole" : "partial"));
    ...
    for (var i = startField; i < PhoneNumber._fieldOrder.length; i++) {
      fieldName = PhoneNumber._fieldOrder[i]; // console.info("format: formatting field " + fieldName + " value: " + number[fieldName]);
      ...
      if (number[fieldName] !== undefined) {
        if (styleTemplates[fieldName] !== undefined) {
          ...
          lastFieldName = fieldName; // console.info("format: formatting field " + fieldName + " with templates " + JSON.stringify(templates));

          temp = this._substituteDigits(number[fieldName], templates, fieldName === "subscriberNumber"); // console.info("format: formatted is: " + temp);
   ...
   * example 
   */
  getStyleExample: function getStyleExample(style) {
