...
var Name = function Name(name, options) {
  var sync = true;
          ...
          Utils.loadData({
            ...
            callback: ilib.bind(this, function (info) {
              ...
              if (_typeof(name) === 'object') {
                ...
                this.honorific = name.honorific; // private properties
...
Name.defaultInfo = {
  "western": ilib.data.name || {
    ...
    "prefixes": ["doctor", "dr", "mr", "mrs", "ms", "mister", "madame", "madamoiselle", "miss", "monsieur", "señor", "señora", "señorita"],
    "suffixes": [",", "junior", "jr", "senior", "sr", "i", "ii", "iii", "esq", "phd", "md"],
    "patronymicName": [],
    "familyNames": []
  },
  "asian": {
...
Name._isAsianChar = function (c) {
  return isIdeo(c) || CType.withinRange(c, "hangul") || CType.withinRange(c, "hiragana") || CType.withinRange(c, "katakana");
};
/**
 * @static
 * @protected
...
Name.prototype = {
   ...
   * @protected
   */
  _init: function _init(name) {
    var parts, prefixArray, prefix, prefixLower, suffixArray, suffix, suffixLower, i, info, hpSuffix;
    ...
    if (name) {
      ...
      if (parts.length > 1) {
        for (i = parts.length; i > 0; i--) {
          ...
          if (ilib.isArray(this.info.prefixes) && (JSUtils.indexOf(this.info.prefixes, prefixLower) > -1 || this._isConjunction(prefixLower))) {
            if (this.prefix) {
              if (!this.isAsianName) {
                this.prefix += ' ';
              }
  ...
  */
   ...
   */
  _findPrefix: function _findPrefix(parts, names, isAsian, noCompoundPrefix) {
    var i,
        prefix,
        prefixLower,
        prefixArray,
   ...
   * @protected
   */
  _findSuffix: function _findSuffix(parts, names, isAsian) {
    var i,
        j,
        seq = "";
  ...
  _isConjunction: function _isConjunction(word) {
    return this.info.conjunctions.and1 === word || this.info.conjunctions.and2 === word || this.info.conjunctions.or1 === word || this.info.conjunctions.or2 === word || "&" === word || "+" === word;
  },
  ...
  _findLastConjunction: function _findLastConjunction(parts) {
    ...
    for (index = 0; index < parts.length; index++) {
      ...
      if (typeof part === 'string') {
        part = part.toLowerCase(); // also recognize English
   ...
   */
  _extractPrefixes: function _extractPrefixes(parts, isAsian) {
   ...
   */
  _extractSuffixes: function _extractSuffixes(parts, isAsian) {
   ...
   */
  _joinAuxillaries: function _joinAuxillaries(parts, isAsian) {
  ...
  _joinNameArrays: function _joinNameArrays() {
    ...
    for (prop in this) {
      if (this[prop] !== undefined && _typeof(this[prop]) === 'object' && ilib.isArray(this[prop])) {
   ...
   * @protected
   */
  _parseAsianName: function _parseAsianName(parts, language) {
    ...
    if (familyNameArray && familyNameArray.length > 0) {
      ...
      this.givenName = parts.slice(this.familyName.length).join(''); //Overide parsing rules if spaces are found in korean
   ...
   * @protected
   */
  _parseKoreanName: function _parseKoreanName(name) {
    var tempName = name;
   ...
   * @protected
   */
  _parseJapaneseName: function _parseJapaneseName(parts) {
   ...
   * @protected
   */
  _parseSpanishName: function _parseSpanishName(parts) {
    var conjunctionIndex;

    if (parts.length === 1) {
      if (this.prefix || _typeof(parts[0]) === 'object') {
        this.familyName = parts[0];
      } else {
        this.givenName = parts[0];
      }
      ...
      conjunctionIndex = this._findLastConjunction(parts); // if there's an 'and' in the middle spot, put everything in the first name
      ...
      conjunctionIndex = this._findLastConjunction(parts); ////console.log("@@@@@@@@@@@@@@@@"+conjunctionIndex)
   ...
   * @protected
   */
  _parseIndonesianName: function _parseIndonesianName(parts) {
    var conjunctionIndex;
    ...
    if (parts.length === 1) {
      ...
      if (conjunctionIndex > 0) {
        ...
        this.givenName = parts.splice(0, 1); //this.familyName = parts.splice(parts.length-2, 2);
   ...
   * @protected
   */
  _parseGenericWesternName: function _parseGenericWesternName(parts) {
    ...
    if (parts.length === 1) {
      ...
      if (conjunctionIndex > 0) {
        ...
        if (conjunctionIndex + 1 < parts.length - 1) {
          this.familyName = parts.splice(parts.length - 1, 1); ////console.log(this.familyName);
  ...
  */
  _findPatronymicName: function _findPatronymicName(parts) {
    var index, part;
    ...
    for (index = 0; index < parts.length; index++) {
      ...
      if (typeof part === 'string') {
        ...
        while (subLength--) {
          if (part.indexOf(this.info.patronymicName[subLength]) !== -1) return index;
        }
      }
    }
  ...
  */
  _isPatronymicName: function _isPatronymicName(part) {
    var pName;
    ...
    if (typeof part === 'string') {
      ...
      while (subLength--) {
        if (pName.indexOf(this.info.patronymicName[subLength]) !== -1) return true;
      }
    }
  ...
  */
  _findFamilyName: function _findFamilyName(parts) {
    ...
    for (index = 0; index < parts.length; index++) {
      ...
      if (typeof part === 'string') {
        ...
        } else if (part[length - 1] === 'в' && part[length] === 'а' || part[length - 1] === 'н' && part[length] === 'а' || part[length - 1] === 'а' && part[length] === 'я') {
  ...
  * @return
  */
  _parseRussianName: function _parseRussianName(parts) {
    var conjunctionIndex,
        familyIndex = -1;
   ...
   * @protected
   */
  _parseWesternName: function _parseWesternName(parts) {
   ...
   */
  getSortFamilyName: function getSortFamilyName() {
    var name, auxillaries, auxString, parts, i; // no name to sort by
      ...
      if (this.info.sortByHeadWord) {
        ...
        if (auxillaries && auxillaries.length > 0) {
          if (typeof this.familyName === 'string') {
            ...
            name = parts.slice(auxillaries.length).join(' ') + ', ' + parts.slice(0, auxillaries.length).join(' ');
          }
        }
  ...
  getHeadFamilyName: function getHeadFamilyName() {},
