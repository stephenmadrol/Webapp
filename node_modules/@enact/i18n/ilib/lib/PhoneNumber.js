...
var PhoneNumber = function PhoneNumber(number, options) {
  var stateData, regionSettings;
  this.sync = true;
  this.loadParams = {};
...
PhoneNumber._parseImsi = function (data, imsi) {
  ...
  while (i < imsi.length) {
    ch = PhoneNumber._getCharacterCode(imsi.charAt(i)); // console.info("parsing char " + imsi.charAt(i) + " code: " + ch);
    ...
    if (ch >= 0) {
      ...
      if (_typeof(newState) === 'object') {
        ...
        if ((typeof newState === 'undefined' || newState === 0 || _typeof(newState) === 'object' && typeof newState.l === 'undefined') && lastLeaf) {
        ...
        if (typeof newState === 'number' && newState || _typeof(newState) === 'object' && typeof newState.l !== 'undefined') {
          // final state
          var stateNumber = typeof newState === 'number' ? newState : newState.l;
          handlerMethod = PhoneNumber._states[stateNumber]; // console.info("reached final state " + newState + " handler method is " + handlerMethod + " and i is " + i);
...
PhoneNumber._states = ["none", "unknown", "plus", "idd", "cic", "service", "cell", "area", "vsc", "country", "personal", "special", "trunk", "premium", "emergency", "service2", "service3", "service4", "cic2", "cic3", "start", "local"];
/**
 * @private
 */

PhoneNumber._fieldOrder = ["vsc", "iddPrefix", "countryCode", "trunkAccess", "cic", "emergency", "mobilePrefix", "serviceCode", "areaCode", "subscriberNumber", "extension"];
...
PhoneNumber.prototype = {
   ...
   */
  _parseOtherCountry: function _parseOtherCountry(number, regionData, options, countryCode) {
    new PhoneLocale({
      locale: this.locale,
   ...
   * @param {Object} options
   */
  _parseNumber: function _parseNumber(number, regionData, options) {
    var i,
        ch,
        regionSettings,
        newState,
    ...
    while (i < number.length) {
      ...
      if (ch >= 0) {
        ...
        if (_typeof(newState) === 'object' && i + 1 < number.length) {
          if (typeof newState.l !== 'undefined') {
          ...
          if (typeof newState === 'number' && newState || _typeof(newState) === 'object' && typeof newState.l !== 'undefined') {
            ...
            i = consumed = 0;
            lastLeaf = undefined; //console.log("reparsing with new number: " +  number);

            currentState = regionSettings.stateData; // if the handler requested a special sub-table, use it for this round of parsing,
            // otherwise, set it back to the regular table to continue parsing

            if (result.countryCode !== undefined) {
              this._parseOtherCountry(number, regionData, options, result.countryCode); // don't process any further -- let the work be done in the onLoad callbacks
            ...
            handlerMethod = typeof newState === 'number' ? "none" : "local"; // failed parse. Either no last leaf to fall back to, or there was an explicit
    ...
    if (i >= number.length && currentState !== regionData.stateData) {
      handlerMethod = typeof currentState.l === 'undefined' || currentState === 0 ? "none" : "local"; // we reached the end of the phone number, but did not finish recognizing anything. 
   ...
   * @protected
   */
  _hasPrefix: function _hasPrefix() {
    ...
    try {
      for (var field in PhoneNumber._fieldOrder) {
        if (typeof field === 'string' && typeof PhoneNumber._fieldOrder[field] === 'string') {
          fieldName = PhoneNumber._fieldOrder[field]; // console.info("normalize: formatting field " + fieldName);
    ...
    if (this._xor(this.countryCode, other.countryCode)) {
      ...
      switch (this.locale.getRegion()) {
        ...
        default:
          ...
          if (this.countryCode !== undefined && this.countryCode !== currentCountryCode || other.countryCode !== undefined && other.countryCode !== currentCountryCode) {
            match -= 16;
          }
   ...
   */
  _doNormalize: function _doNormalize(options, norm, homeLocale, currentLocale, currentPlan, destinationLocale, destinationPlan, sync, loadParams) {
    var formatted = "";
    ...
    if (!norm.invalid && options && options.assistedDialing) {
      ...
      if (norm.subscriberNumber && (!options.manualDialing || norm.iddPrefix || norm.countryCode || norm.trunkAccess)) {
        ...
        if (currentLocale.getRegion() !== destinationLocale.getRegion()) {
          ...
          if (!norm._hasPrefix() && options.defaultAreaCode && destinationLocale.getRegion() === homeLocale.getRegion() && (!destinationPlan.getFieldLength("minLocalLength") || norm.subscriberNumber.length >= destinationPlan.getFieldLength("minLocalLength"))) {
          ...
          if (options.sms && homeLocale.getRegion() === "US" && currentLocale.getRegion() !== "US") {
   ...
   */
  _doReparse: function _doReparse(options, norm, homeLocale, currentLocale, currentPlan, destinationLocale, destinationPlan, sync, loadParams, callback) {
    var formatted, tempRegion;

    if (options && options.assistedDialing && !norm.trunkAccess && !norm.iddPrefix && norm.subscriberNumber && norm.subscriberNumber.length > destinationPlan.getFieldLength("maxLocalLength")) {
