...
var DateRngFmt = function DateRngFmt(options) {
  var sync = true;
  ...
  opts.onLoad = ilib.bind(this, function (fmt) {
    ...
    if (fmt) {
      this.locinfo = this.dateFmt.locinfo; // get the default calendar name from the locale, and if the locale doesn't define
...
DateRngFmt.prototype = {
   ...
   */
  format: function format(startDateLike, endDateLike) {
    var startRd,
        endRd,
        fmt = "",
        yearTemplate,
    ...
    if (_typeof(start) !== 'object' || !start.getCalendar || start.getCalendar() !== this.calName || _typeof(end) !== 'object' || !end.getCalendar || end.getCalendar() !== this.calName) {
