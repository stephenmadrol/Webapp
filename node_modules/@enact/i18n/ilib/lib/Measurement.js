...
var Measurement = function Measurement(options) {
  if (options) {
    ...
    if (_typeof(options.amount) === 'object') {
...
Measurement.prototype = {
   ...
   */
  normalizeUnits: function normalizeUnits(name) {
    return Measurement.getUnitId(this.constructor, name) || Measurement.getUnitIdCaseInsensitive(this.constructor, name) || name;
  },
   ...
   * @return {string} name of the unit of measurement
   */
  getOriginalUnit: function getOriginalUnit() {
   ...
   */
  getMeasurementSystem: function getMeasurementSystem() {
  ...
  localize: function localize(locale) {
    ...
    to = this.systems.conversions[fromSystem] && this.systems.conversions[fromSystem][toSystem] && this.systems.conversions[fromSystem][toSystem][this.unit];
   ...
   * be converted
   */
  convertSystem: function convertSystem(measurementSystem) {
   ...
   * right level
   */
  scale: function scale(measurementsystem, units) {
    var systemName = this.getMeasurementSystem();
    var mSystem;

    if (units) {
      mSystem = units[measurementsystem] && JSUtils.indexOf(units[measurementsystem], this.unit) > -1 ? units[measurementsystem] : units[systemName];
    }

    if (!mSystem) {
      mSystem = this.systems[measurementsystem] && JSUtils.indexOf(this.systems[measurementsystem], this.unit) > -1 ? this.systems[measurementsystem] : this.systems[systemName];
    }
   ...
   */
  expand: function expand(measurementsystem, units, constrain, scale) {
    var systemName = this.getMeasurementSystem();
   ...
   * their amounts
   */
  list: function list(measures, ratios, constrain, scale) {
    ...
    amount = constrain(amount); // go backwards so we get from the largest to the smallest units in order
   ...
   * @private
   */
  scaleUnits: function scaleUnits(mSystem) {
    var tmp,
        munit,
        amount = 18446744073709551999;
   ...
   */
  getUnitId: function getUnitId(unit) {
   ...
   * undefined if there is no such unit in this type of measurement
   */
  getUnitIdCaseInsensitive: function getUnitIdCaseInsensitive(unit) {
    if (!unit) return false; // try with the original case first, just in case that works
...
Measurement.getUnitIdCaseInsensitive = function (measurement, unit) {
  ...
  var u = unit.toLowerCase(); // try this first, just in case
...
Measurement.getMeasurementSystemForLocale = function (locale) {
  var l = _typeof(locale) === "object" ? locale : new Locale(locale);
