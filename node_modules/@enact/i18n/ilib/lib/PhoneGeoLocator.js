...
var PhoneGeoLocator = function PhoneGeoLocator(options) {
  var sync = true,
      loadParams = {},
...
PhoneGeoLocator.prototype = {
   ...
   * @returns {Object}
   */
  _getAreaInfo: function _getAreaInfo(number, data, locale, plan, options) {
    var sync = true,
        ret = {},
        countryCode,
        areaInfo,
    ...
    if (prefix !== undefined) {
      if (plan.getExtendedAreaCode()) {
        ...
        Utils.loadData({
          ...
          locale: locale,
          sync: sync,
          loadParams: JSUtils.merge(options && options.loadParams || {}, {
            returnOne: true
          }),
          callback: ilib.bind(this, function (data) {
            this.extarea = data;
            Utils.loadData({
              ...
              callback: ilib.bind(this, function (data) {
                ...
                if (!plan.fieldLengths || plan.getFieldLength('maxLocalLength') === undefined || !number.subscriberNumber || number.subscriberNumber.length <= plan.fieldLengths('maxLocalLength')) {
      ...
      } else if (!plan || plan.getFieldLength('maxLocalLength') === undefined || !number.subscriberNumber || number.subscriberNumber.length <= plan.getFieldLength('maxLocalLength')) {
        if (geoTable) {
   ...
   */
  locate: function locate(number, options) {
    var loadParams = {},
        ret = {},
        region,
        countryCode,
    ...
    if (number === undefined || _typeof(number) !== 'object' || !(number instanceof PhoneNumber)) {
      return ret;
    }
   ...
   * @return {string}
   */
  country: function country(number) {
    var countryCode, region, phoneLoc;
    ...
    region = number.countryCode && phoneLoc._mapCCtoRegion(number.countryCode) || number.locale && number.locale.region || phoneLoc.locale.getRegion() || this.locale.getRegion();
